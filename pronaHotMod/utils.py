...
def read_prona(f):
    ...
    for l in open(f):
        ...
        if '#' in l:
            continue
        t = l.split('\t')
        if 'Protein_level' in l:
            protein_level_Pro=int(t[2])
            protein_level_DNA=int(t[4])
            protein_level_RNA=int(t[6])
        elif 'Res_' in l:
            Res=str(int(t[0].split('_')[-1])-1)
            Pro=int(t[3])*protein_level_Pro
            DNA=int(t[5])*protein_level_DNA
            RNA=int(t[7])*protein_level_RNA
#            Pro_RI=int(t[2])*protein_level_Pro[pr]
#            DNA_RI=int(t[4])*protein_level_DNA[pr]
#            RNA_RI=int(t[6])*protein_level_RNA[pr]
#            Pro_RI=(Pro_RI-ct_Pro)/(1-ct_Pro)
#            DNA_RI=(DNA_RI-ct_DNA)/(1-ct_DNA)
#            RNA_RI=(RNA_RI-ct_RNA)/(1-ct_RNA)
            residue_level_Pro.append(Pro)
            residue_level_DNA.append(DNA)
            residue_level_RNA.append(RNA)
    return residue_level_Pro,residue_level_DNA,residue_level_RNA
...
def dct(pssm, x, y, v, u, V, U):
    ...
    return alpha(u,U)*alpha(v,V)*pssm[y,x]*cos(((2*x+1)*(u*pi))/(2*U))*cos(((2*y+1)*(v*pi))/(2*V))

def new_dct(pssm,win):
    halfWin=(win-1)/2
    pssm=np.array(pssm[0])
    maxV = pssm.shape[0] #y
    maxU = pssm.shape[1] #x

    imageSet = np.zeros((maxV,maxU))
    for u in range(0, maxU):
        for v in range(0, maxV):
            basisImg = 0
            for y in range(0, maxV):
                for x in range(0, maxU):
                    basisImg += dct(pssm,x, y, v, u, maxV, maxU)
            imageSet[v,u]=basisImg

    pssm_dct=[]

    for t in range(int(len(imageSet))):
        if t==halfWin:
            pssm_dct.extend(list(imageSet[t]))
        elif t<halfWin:
            pssm_dct.extend(list(imageSet[t])[0:10])
        else:
            pssm_dct.extend(list(imageSet[t])[10:])
...
#haar
    ...
    AL4,DH4,DH3,DH2,DH1=pywt.wavedec(pssm,w,m,level=4)
    pssm_dwt=[]

    for t in AL4:
        pssm_dwt.extend(list(t))
    for t in DH4:
        pssm_dwt.extend(list(t))
