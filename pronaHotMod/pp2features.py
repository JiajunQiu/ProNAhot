...
def extract_features(pp_path ,feature_file):
	...
	#5.5 Consurf?
	try:
		consurf = open(pp_path+"query.consurf.grades").read()
	except IOError:
		print("%s: query.consurf.grades not found, skipping...\n" % pro)
		os.exit()
	parsed_consurf = parse_consurf(consurf)
	#We can only check here for identical lengths since bval doesn't return a sequence
	if len(seq) != len(parsed_consurf['consurf_score']) != len(parsed_consurf['consurf_color']):
		sys.exit("%s: Disagreement between pp sequence and consurf sequence, skipping...\n" % pro)
	...
	for pos in range(len(seq)):
		...
		#5.5 consurf features
		if parsed_consurf:
			featnames_consurf, window_consurf = feature_consurf(parsed_consurf, pos, fea_win, norm_raw=consurf_norm)
			instance_feature_names.update(featnames_consurf)
			instance_features.update(window_consurf)	
